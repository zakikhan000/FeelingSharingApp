...
public class NotificationActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        new Handler(Looper.getMainLooper()).postDelayed(this::markAllNotificationsAsRead, 10000);
    }
    ...
    private void handleNotificationClick(Notification notification) {
        ...
        switch (notification.getType()) {
            ...
            case "comment":
                ...
                Intent broadcastIntent = new Intent("com.example.shadowchatapp2.POST_NOTIFICATION");
                broadcastIntent.putExtra("post_id", notification.getReferenceId());
                broadcastIntent.putExtra("type", "comment");
                sendBroadcast(broadcastIntent);
                finish(); // Close NotificationActivity after sending broadcast
                break;
            case "like":
                // Send broadcast to HomeFragment
                broadcastIntent = new Intent("com.example.shadowchatapp2.POST_NOTIFICATION");
                broadcastIntent.putExtra("post_id", notification.getReferenceId());
                broadcastIntent.putExtra("type", "like");
                sendBroadcast(broadcastIntent);
                finish(); // Close NotificationActivity after sending broadcast
                break;
            default:
    ...
    private void markAllNotificationsAsRead() {
        ...
        Request request = new Request.Builder()
                .url(url)
                .post(RequestBody.create(null, new byte[0])) // Empty POST
                .build();
